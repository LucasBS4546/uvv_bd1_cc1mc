...
/*
Primeiro, inicia-se o processo de criar o banco de dados 'uvv' e um usuário 'lucasbonatosoares'.
Cria-se o usuário, para então criar o banco de dados. Conecta-se ao banco de dados e muda-se o usuário para o usuário criado. 

No mariaDB, não há necessidade de escrever comandos "DROP IF EXIST" para o banco de dados e usuário antes de cria-los, pois esse SGBD suporta o uso do comando "CREATE OR REPLACE" que faz as operações de drop e create juntas.
No mariaDB, não existe o conceito de esquemas, logo não será feito a criação de um esquema 'lojas' neste banco de dados. Todas as estruturas e objetos estarão armazenados diretamente no banco de dados.
*/



-- Criação do usuário
CREATE OR REPLACE USER lucasbonatosoares
IDENTIFIED BY 'banana';


-- Criação do banco de dados 'uvv'
DROP DATABASE uvv;
CREATE OR REPLACE DATABASE uvv
    CHARACTER SET = 'UTF8'
    COMMENT = 'Contêm dados relacionados às lojas UVV e todos os seus serviços, processos, clientes, etc.';
